...
public class PassiveLocationChangedReceiver extends BroadcastReceiver {
    ...
    protected static void processLocation(final Context context, final Location location) {
        ...
        final int lastAccuracy =
                prefs.getInt(LocationLibraryConstants.SP_KEY_LAST_LOCATION_UPDATE_ACCURACY, Integer.MAX_VALUE);
        ...
        if (lastLat != Long.MIN_VALUE) {
            ...
            int distanceBetweenInMetres = (int) (Math
                    .acos(Math.sin(Math.toRadians(thisLat)) * Math.sin(Math.toRadians(lastLat)) +
                            Math.cos(Math.toRadians(thisLat)) * Math.cos(Math.toRadians(lastLat)) *
                                    Math.cos(Math.toRadians(thisLong) - Math.toRadians(lastLong))) * 6371 * 1000);

            if (LocationLibrary.showDebugOutput) {
                Log.d(LocationLibraryConstants.TAG,
                        TAG + ": Distance from last reading: " + distanceBetweenInMetres + "m");
            }
        ...
        if (!usePreviousReading) {
            ...
            if (LocationLibrary.showDebugOutput) Log.d(LocationLibraryConstants.TAG,
                    TAG + ": Storing location update, lat=" + thisLat + " long=" + thisLong + " accuracy=" +
                            thisAccuracy + " time=" + thisTime + "(" + DateFormat.format("kk:mm.ss, E", thisTime) +
                            ")");
        } else {
            if (LocationLibrary.showDebugOutput) Log.d(LocationLibraryConstants.TAG,
                    TAG + ": Storing location update, less accurate so reusing prior location - time=" + thisTime);
        }
        prefsEditor.commit();
